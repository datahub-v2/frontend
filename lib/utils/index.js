...
module.exports.prettifyBytes = dpjson => {
  dpjson.datahub.stats.prettyBytes = bytes(dpjson.datahub.stats.bytes)
  dpjson.resources.forEach(resource => {
    resource.prettyBytes = bytes(resource.bytes)
    if (resource.alternates) {
      resource.alternates.forEach(res => {
        res.prettyBytes = bytes(res.bytes)
      })
    }
  })
  return dpjson
...
module.exports.normalize = extendedDp => {
  const normalizedDp = JSON.parse(JSON.stringify(extendedDp))
  normalizedDp.resources = []
  extendedDp.resources.forEach(resource => {
    if (resource.datahub.type.startsWith('source')) {
      normalizedDp.resources.push(resource)
    }
  })

  const newListOfResources = []
  normalizedDp.resources.forEach(original => {
    let newResource
    extendedDp.resources.forEach(resource => {
      if (resource.datahub.derivedFrom) {
  ...
  normalizedDp.resources = newListOfResources
  return normalizedDp
}
