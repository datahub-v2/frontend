...
/**
* Generates list of datapackages
* @param {Array} array of package owner and name objects: [{owner: '', name: ''}]
* @return {Array} array of promises that evaluates to datapackages
*/
module.exports.getListOfDatapackages = async arrayOfPkgId => {
  const api = new lib.DataHubApi(config)
  return Promise.all(arrayOfPkgId.map(pkgId => {
    return api.getPackage(pkgId.owner, pkgId.name)
  }))
}

/**
* Generates list of short readme
* @param {Array} array of package owner and name objects: [{owner: '', name: ''}]
* @return {Array} array of promises that evaluates to short readme
*/
module.exports.getListOfReadme = async arrayOfPkgId => {
  const api = new lib.DataHubApi(config)
  return Promise.all(arrayOfPkgId.map(pkgId => {
    return api.getPackageFile(pkgId.owner, pkgId.name, 'README.md')
  }))
}

/**
* Generates list of datapackages with readme and owner info
* @param {Array} array of package owner and name objects: [{owner: '', name: ''}]
* @return {Array} array of datapackages with readme and owner info
*/
module.exports.getListOfDpWithReadme = async arrayOfPkgId => {
  let listOfPackages = await module.exports.getListOfDatapackages(arrayOfPkgId)
  listOfPackages = listOfPackages.filter(pkg => pkg)
  let listOfReadme = await module.exports.getListOfReadme(arrayOfPkgId)
  listOfReadme = listOfReadme.filter(readme => readme)
  for (let i = 0; i < listOfPackages.length; i++) {
    const shortReadme = module.exports.makeSmallReadme(listOfReadme[i])
    listOfPackages[i].shortReadme = shortReadme
    listOfPackages[i].owner = listOfPackages[i].owner || {username: arrayOfPkgId[i].owner}
  }
