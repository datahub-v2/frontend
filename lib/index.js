  ...
  */
  ...
  async search(query, token) {
    ...
    response.results.map(res => {
      res.datapackage = DataHubApi.extendedToLogical(res.datapackage)
      // Do preparation for frontend, e.g., prettify bytes
      res.datapackage = utils.prepareForFrontend(res.datapackage)
      if (res.datapackage.readme) { // Convert readme to plain text and shorten
        res.datapackage.readmeSnippet = utils.makeSmallReadme(res.datapackage.readme)
      }
      return res
    })
    return response
   ...
   */
  async pipelineStatus(ownerid, name) {
    const url = `${this.DATAHUB_API}/source/${ownerid}/${name}/status`
    const response = await fetch(url)
    if (response.status !== 200) {
      throw response
    }
  ...
  static extendedToLogical(extended) {
    ...
    extended.resources.forEach(resource => {
      if (resource.datahub && resource.datahub.type.startsWith('source')) {
        logical.resources.push(resource)
      } else if (resource.datahub && resource.datahub.type === 'non-tabular') {
