   ...
   */
   static async makeGoodDp(dp) {
     ...
     if (good.report) {
       ...
       good.report.valid = true
       let response = await fetch(good.report.path)
       if (response.status === 200) {
         const out = await response.json()
         out.forEach(report => {
           const resourceForThisReport = good.resources.find(res => res.name === report.resource)
